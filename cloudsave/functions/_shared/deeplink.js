...
const APP_SCHEME = "cloudsave://";
const IOS_APP_STORE = "https://apps.apple.com/app/cloudsave/id6757381326";
const ANDROID_PLAY_STORE = "https://play.google.com/store/apps/details?id=org.cloudsave.app";
const APP_STORE_ID = "6757381326";

export function generateDeepLinkHTML(options) {
  const { path, title = "Opening CloudSave...", description = "You're being redirected to the CloudSave app." } = options;
...
<body>
  <div class="container">
    ...
    <div id="fallback" class="hidden">
      <h1>Get CloudSave App</h1>
      <p>Download the CloudSave app to view this content.</p>
      <div class="buttons">
        <a href="${IOS_APP_STORE}" class="btn btn-primary" id="ios-btn">
          Download on App Store
        </a>
        <a href="${ANDROID_PLAY_STORE}" class="btn btn-primary" id="android-btn">
          Get it on Google Play
        </a>
        <a href="#" class="btn btn-secondary" id="open-app">
          I have the app installed
        </a>
      </div>
    </div>
    <p class="footer">CloudSave - Secure Cloud Storage</p>
  </div>
  <script>
    const appSchemeUrl = "${appSchemeUrl}";
    const iosStore = "${IOS_APP_STORE}";
    const androidStore = "${ANDROID_PLAY_STORE}";
    const deepLinkPath = "${path}";
    ...
    function init() {
      ...
      // iOS: Universal Links should handle it automatically
      // If we're here, page is loading which means UL might have failed
      // Just wait briefly and show fallback - NO iframe hack, NO auto-redirect
      if (platform === 'ios') {
        setTimeout(() => {
          if (!document.hidden) {
            showFallback();
      ...
      // Android: Use Intent URL (reliable with built-in fallback)
      if (platform === 'android') {
        const intentUrl = 'intent:/' + deepLinkPath + '#Intent;scheme=cloudsave;package=org.cloudsave.app;S.browser_fallback_url=' + encodeURIComponent(androidStore) + ';end';
    ...
    // Manual "I have the app" button
    document.getElementById('open-app').addEventListener('click', (e) => {
      e.preventDefault();
      const platform = detectPlatform();
      if (platform === 'android') {
        const intentUrl = 'intent:/' + deepLinkPath + '#Intent;scheme=cloudsave;package=org.cloudsave.app;end';
        window.location.href = intentUrl;
      } else {
